...
class Data:
    ...
    def read(self, filepath):
        ...
        # parse the data to only store numeric columns
        ...
        for i in range(len(data[1])-1,-1,-1):
            ...
            if data[1][i] == 'string':
                string_data.append(data[2:,i])       
            if data[1][i] != 'numeric':
                data = np.delete(data,i,axis = 1)
        self.string_data = np.array(string_data).T
        ...
        self.header2col = {}
        counter = 0
        for u in self.headers:
            self.header2col[u] = counter
            counter+=1
...
Headers:
  '''%(self.filepath, self.data.shape[0], self.data.shape[1])
        ...
        if self.data.shape[0]>=5:
            ...
            numRow = self.data.shape[0]
            string += "Showing first %d rows." % (self.data.shape[0]) +"\n"
    ...
    def get_header_indices(self, headers):
        ...
        if isinstance(headers,list):
            for i in headers:
                header_indicies.append(self.header2col[i])
        else:
            header_indicies.append(self.header2col[headers])
