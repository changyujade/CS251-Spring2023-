...
class Data:
    def __init__(self, filepath=None, headers=None, data=None, header2col=None):
        ...
        self.headers = headers # a list
        self.header2col = header2col # a dictionary
    ...
    def read(self, filepath):
        ...
        with open(filepath, 'r') as csvfile:
            csvreader = csv.reader(csvfile) #ignore non-numeric??
            data = np.array(list(csvreader))
        
        self.filepath = filepath

        types = ['numeric','string','date','enum']
        # np_list = data[1].tolist
        if any(i.strip() not in types for i in data[1]):
            sys.exit("Does not contain correct file types")
        

        # Read the first line and split it by commas, to get the list of headers
        strings = []
        numerics = []
        ...
        for i in range(len(data[1])-1,-1,-1):
            ...
            if data[1][i] == 'numeric':
                numerics.insert(0,data[2:,i])
            elif data[1][i] == 'string':
                strings.insert(0,data[2:,i])
        
        
        self.strings = np.array(strings).T
        numerics = np.array(numerics).T
        
        self.headers = data[0].tolist()

        self.header2col = {}
        counter = 0
        for i in self.headers:
            self.header2col[i] = counter
            counter +=1
        # delete the first two rows of headers, then store the rest of the data in self.data
        self.data = np.array(numerics,np.float64)
        
        # return np.array(data[2:,:],np.float64)
...
Headers:
  '''%(self.filepath, self.data.shape[0], self.data.shape[1])
        ...
        if self.data.shape[0]>=5:
            string += "Showing first 5/%d rows." % (self.data.shape[0]) +"\n"
    ...
    def get_mappings(self):
        # add another parameter called variable name?
    ...
    def get_header_indices(self, headers):
        ...
        if isinstance(headers,list):
            ...
            header_indices.append(self.header2col[headers])
            #header_indices.append(self.header2col[headers])
    ...
    def select_data(self, headers, rows=[]):
        ...
        '''
        header_index = self.get_header_indices(headers)
        if rows != []:
            select = self.data[np.ix_(rows,header_index)]
        else:
            #select = self.data[np.ix_(list(range(self.data.shape[0])),header_index)]
            select = self.data[np.ix_(list(range(self.data.shape[0])),header_index)]
        return select
