...
class Analysis:
    ...
    def min(self, headers, rows=[]):
        ...
        d = self.data.select_data(headers,rows)
    ...
    def var(self, headers, rows=[]):
        ...
        d = self.data.select_data(headers,rows)
        sum = d.sum(axis = 0)
        vars  = (np.sum((d - self.mean(headers,rows))**2, axis = 0))/(np.shape(d)[0]-1)
        return vars
    ...
    def pair_plot(self, data_vars, fig_sz=(12, 12), title=''):
        ...
        for row, element in enumerate(data_vars):
            for col, element_y in enumerate(data_vars):
                # plt.subplots(length_of_data,length_of_data,(i*length_of_data)+(j+1))
                # self.scatter(element, element_y,title)
                ax[row,col].scatter(self.data.select_data(element),self.data.select_data(element_y))
                if row == length_of_data - 1:
                    ax[row,col].set_xlabel(element)
                if col == 0:
