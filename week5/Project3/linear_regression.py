...
class LinearRegression(analysis.Analysis):
    ...
    def scatter(self, ind_var, dep_var, title):
        ...
        xline = np.linspace(x.min(), x.max()) # x values on the line
        xline = xline.reshape(xline.shape[0],1)
        
        if self.p >1:
            rline = self.intercept + self.make_polynomial_matrix(xline,self.p) @ self.slope
        else:
            rline = self.intercept + xline*self.slope
        plt.plot(xline, np.squeeze(rline), 'r')
        plt.title(title+( f" R2 = {self.R2:0.3f}"))
    ...
    def pair_plot(self, data_vars, fig_sz=(12, 12), hists_on_diag=True):
        ...
        for row, element_y in enumerate(data_vars):
            for col, element_x in enumerate(data_vars):
                ...
                A = self.data.select_data(element_x)
                y = self.data.select_data(element_y)

                self.linear_regression(element_x,element_y)
    
                xline = np.linspace(A.min(), A.max()) # x values on the line
                rline = self.c[0] + self.c[1]*xline # the y values on the line
                ax[row,col].plot(xline, np.squeeze(rline), 'r')
                ax[row,col].set_title(f" R2 = {self.R2:0.3f}")

                if row == col and hists_on_diag is True:
                    numVars = len(data_vars)
                    ax[row, col].remove()
                    ax[row, col] = fig.add_subplot(numVars, numVars, row*numVars+col+1)
                    # We can set the number of bins with the *bins* keyword argument.
                    ax[row,col].hist(A)
                    if row < numVars-1:
                        ax[row, col].set_xticks([])
                    else:
                        ax[row, col].set_xlabel(data_vars[row])
                    if col > 0:
                        ax[row, col].set_yticks([])
                    else:
                        ax[row, col].set_ylabel(data_vars[row])
    ...
    def make_polynomial_matrix(self, A, p):
        ...
        degree_list = np.arange(1,p+1)
        Ahat = A**degree_list
        ## TODO: maybe need to turn it into float ?
        return Ahat
    ...
    def poly_regression(self, ind_var, dep_var, p):
        ...
        y = self.data.select_data(dep_var)
        poly_matrix = self.make_polynomial_matrix(A,p)
        Ahat = np.hstack((np.ones((A.shape[0],1)),poly_matrix)) 
        c, _, _, _ = lstsq(Ahat, y)
